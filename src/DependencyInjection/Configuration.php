        ...
        $rootNode
            ...
            ->children()
                ...
                ->booleanNode('recalculate_on_doctrine_flush')
                    ->info('Whether to automatically recalculate the completeness of products affected by a Doctrine flush')
                    ->defaultTrue()
                ->end()
                ->integerNode('bulk_threshold')
                    ->info('Number of affected products in a single flush above which a single catalog-wide recalculation is dispatched instead of per-product recalculations')
                    ->defaultValue(300)
                    ->min(1)
                ->end()
            ->end();
